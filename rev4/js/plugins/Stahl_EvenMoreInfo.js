 ...
 * @returns Custom Line Object
 */
StatusDisplayManager.createLinePair = function(label, text, labelColor = "#ffff55", color = "#ffffff", gap = 50) {
  return {lineType: "PAIR", label: label, text: text, labelColor: labelColor, color: color, gap: gap}
}
...
Sprite_EnemyBattlerStatus.prototype.drawMoreInfoLines = function(infoLines) {
  ...
  for (let i = 0; i < infoLines.length; i++) {
    ...
    if (line.lineType == "SIMPLE") {
      ...
      this.bitmap.textColor = line.color;
      this.bitmap.drawText(line.text, 8 + line.gap, curY, this.width, 12, 'left');
    } else {
